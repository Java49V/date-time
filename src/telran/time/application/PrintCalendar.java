...
public class PrintCalendar {
    ...
    private static final DayOfWeek DEFAULT_FIRST_WEEK_DAY = DayOfWeek.MONDAY;
    ...
    private static int getFirstWeekDay(int month, int year, DayOfWeek firstWeekDay) {
        ...
        int defaultWeekDayNumber = DEFAULT_FIRST_WEEK_DAY.getValue();
        int diff = (weekDayNumber - defaultWeekDayNumber + 7) % 7;
        return (firstWeekDay.getValue() + diff) % 7;
    }
    ...
    private static void printWeekDays(DayOfWeek firstWeekDay) {
        System.out.print("  ");
        DayOfWeek[] shiftedDaysOfWeek = shiftDaysOfWeek(firstWeekDay);
        Arrays.stream(shiftedDaysOfWeek).forEach(dw ->
                System.out.printf("%s ",
                        dw.getDisplayName(TextStyle.SHORT, Locale.getDefault())));
        System.out.println();
    }

    private static DayOfWeek[] shiftDaysOfWeek(DayOfWeek firstWeekDay) {
        DayOfWeek[] shiftedDaysOfWeek = new DayOfWeek[7];
        int firstWeekDayValue = firstWeekDay.getValue();
        for (int i = 0; i < 7; i++) {
            shiftedDaysOfWeek[i] = DayOfWeek.of((firstWeekDayValue + i) % 7 + 1);
        }
    ...
    private static RecordArguments getRecordArguments(String[] args) throws Exception {
        ...
        DayOfWeek firstWeekDay = args.length > 2 ? getDayOfWeek(args[2]) : DEFAULT_FIRST_WEEK_DAY;
    ...
    private static DayOfWeek getDayOfWeek(String dayOfWeekStr) throws Exception {
        String message = "";
        try {
